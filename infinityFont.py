...
class InfinityFont:
    def __init__(self, filepath: str, game_code: str):
        ...
        if Path(filepath).suffix == ".json":
            ...
            with EndianBinaryFileReader(filepath) as f:
                ...
                self.glyphs = [
                    FontGlyph(f, char=self.font[i], i=i)
    ...
    def reorganize_font(self, out_path: str, font_sheet_filepath: str):
        with Image.open(font_sheet_filepath) as font_sheet:
            ...
            im = Image.new(
                mode="RGB",
                size=[64 * max_width, max_height * (self.entry_count // 64) + 1],
            )

            for idx, glyph in enumerate(self.glyphs):
                im.paste(
                    glyph.get_glyph_im(font_sheet),
                    (max_width * (idx % 64), max_height * (idx // 64)),
                )
...
class FontGlyph:
    ...
    def get_glyph_im(self, font_sheet: Image.Image):
        return font_sheet.crop(
            (self.x_pos, self.y_pos, self.x_pos + self.width, self.y_pos + self.height)
        )
...
def main():
    ...
    if args[1] == "-e":
        ...
        print(
            f"Successfully wrote new font to {args[4]} and imported new characters to font.txt!"
        )
    else:
