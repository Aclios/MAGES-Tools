    ...
    def export_glyphs(self, out_path: str):
        ...
        for idx in range(self.count4):
            glyph_data = self.glyph_data[
                self.glyph_datasize * idx : self.glyph_datasize * (idx + 1)
            ]
            glyph = Image.frombytes(
                mode="L", size=(self.glyph_width, self.glyph_height), data=glyph_data
            )
            im.paste(
                glyph,
                box=((idx % 64) * self.glyph_width, (idx // 64) * self.glyph_height),
            )
        im.save(out_path)
